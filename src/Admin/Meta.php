<?php
//@todo make it strictly typed declare(strict_types=1);
...
class Meta {
	...
	public function init() {
		...
		// Bail out now if we are not in the admin panel.
		if ( ! is_admin() ) {
			return;
		}
	...
	public function comment_column( $column, $id ) {
		...
		//@todo remove this stuff parent::__construct();

		// Output the issue into the column
		$issue = get_comment_meta( $id, 'issues', true );

		$comment_issues[] = 'Some randomissue @todo, make this work as before';

		if ( isset( $comment_issues[$issue] ) ) {
